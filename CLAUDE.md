...
- **Merges into `dev` and `main` require ONE human approval** (enforced by the branch-protection
  rulesets in the repo's CI setup). Agents **open** PRs (`gh pr create`) and **STOP** — a human approves
  and merges. Agents MUST NOT run `gh pr merge` on `dev`/`main`.
